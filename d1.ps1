...
function ConvertFrom-Base64($string) {
   ...
   $Photo_MemoryStream = new-object System.IO.MemoryStream(,$bytes)
   return $Photo_MemoryStream;
}
$a=ConvertFrom-Base64($b)
$DeflatedStream = New-Object IO.Compression.DeflateStream($a, [IO.Compression.CompressionMode]::Decompress)
...
function Compare-SequenceNumber ($SequenceNumber, $AcknowledgementNumber) {
    ...
    if ($AcknowledgementNumber -lt $SequenceNumber) { 
